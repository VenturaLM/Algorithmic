...
public:
	...
	* @post not isVisited()
	*/
	GraphNode(size_t label, T const& item): _label(label), _item(item), _visited(false){}
...
public:
	...
	*/
	GraphEdge(NodeRef& u, NodeRef& v, E const& item): _u(u), _v(v), _item(item){}
	...
	* @post first()==u and second()=v
	*/
	static Ref make(NodeRef& u, NodeRef& v, E const& item)
	{
...
public:
	...
	/** @{*/
		 ...
		 * @pre capacity >= 0
		 */
		WGraph(size_t capacity=0): _nodes(capacity), _a_mat(capacity, std::vector<float>(capacity, std::numeric_limits<float>::infinity())), _curr_node(capacity), _curr_edge(capacity){}
	...
	/**@{*/
		...
		*/
		...
		bool has( NodeRef const& u) const
		{
			...
			return u->label() < capacity() && _nodes[u->label()].get() == u.get();
		}
		...
		*/
		...
		{
			assert( has(u) && has(v) );
			return _a_mat[u->label()][v->label()] < std::numeric_limits<float>::infinity();
		}
		...
		*/
		...
		{
			assert(has_current_edge());
			return GraphEdge<T, float>::make(_nodes[_curr_node],  _nodes[_curr_edge], _a_mat[_curr_node][_curr_edge]);
		}
		...
		/** @{*/
				...
				*/
				void goto_first_edge()
				{
					assert(has_current_node());
					_curr_edge = 0;
					while(_curr_edge<_size && _a_mat[_curr_node][_curr_edge]==std::numeric_limits<float>::infinity())
						++_curr_edge;
				}
				...
				*/
				void goto_edge(NodeRef const& u, NodeRef const& v)
				{
					assert(has(u) && has(v));
