...
template <class T> bool traveling_salesman_greedy(WGraph<T> &g, WGraph<T> &newGraph)
{
	...
	for (auto i = 0; i < g.size() - 1; i++)
	{
		...
		typename WGraph<T>::EdgeRef minimum_weight(g.current_edge());
		size_t u, v;
	...
	typename WGraph<T>::NodeRef finalNode = newGraph.node(g.current_node()->label());
	typename WGraph<T>::NodeRef firstNode = newGraph.node(0);
