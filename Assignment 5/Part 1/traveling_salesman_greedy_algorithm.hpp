...
template <class T> 
float 
traveling_salesman_greedy(WGraph<T> &g, WGraph<T> &newGraph)
{
	...
	//	Gets the weight of the graph:
	float totalWeight = 0.0;

	std::vector< typename WGraph<T>::EdgeRef > edges;
	newGraph.goto_first_node();
	while(newGraph.has_current_node())
	{
		newGraph.goto_first_edge();
		while(newGraph.has_current_edge())
		{
			edges.push_back(newGraph.current_edge());
			totalWeight += newGraph.current_weight();
