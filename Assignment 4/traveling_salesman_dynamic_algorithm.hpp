...
template <class T>
bool
traveling_salesman_dynamic(WGraph<T> & g)
{
    ...
    {
    	std::cout << "Path [" << i << "]: ";
    	for(unsigned int j = 0; j <= g.capacity() - 2; j++)
    	{
    	...
    	path_weight[i] += g.weight( g.node(allPaths[i][g.capacity() - 1]), g.node(0));
    	//	Prints every weight, exposing at the right side the previous weight computed.
    	std::cout  << allPaths[i][g.capacity() - 1] 
                    << ", " << allPaths[i][0] 
    ...
    std::cout   << allPaths[index][g.capacity() - 1]
                << ", " << allPaths[index][0]
    ...
    std::cout   << g.node( allPaths[index][g.capacity() - 1] )->item().name
                <<  " - (" << g.weight( g.node( allPaths[index][g.capacity() - 1] ), g.node(allPaths[index][0]) ) << ") - "
                << g.node(allPaths[index][0])->item().name
                << std::endl;
